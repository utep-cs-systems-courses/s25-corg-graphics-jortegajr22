		...
		u_int fgColorBGR, u_int bgColorBGR)
{
  u_char rows = row;
  while (*string) {
    drawChar5x7(col, rows, *string++, fgColorBGR, bgColorBGR);
    rows += 8;
  }
}
...
void drawString8x12(u_char col, u_char row, char *string, u_int fgColorBGR, u_int bgColorBGR) {
  u_char rows = row;
  while (*string) {
    drawChar8x12(col, rows, *string++, fgColorBGR, bgColorBGR);  // Use an 8x12 font
    rows += 12;  // Adjust for 8x12 font spacing
  }
}
...
/* flipped cols and row with numbers
